...
def emit_profile(lines: list[str], profile: dict[str, Any], args: argparse.Namespace) -> None:
    ...
    lines.extend(
        [
            ...
            "      - |",
            "        set -eu",
            "        chown -R \"$${HERMES_UID}:$${HERMES_GID}\" /opt/hermes /opt/data/profiles/default /workspace",
            "        /opt/hermes/.venv/bin/hermes plugins install --force --enable \"$${HERMES_WIKI_REPO}\"",
            ...
            "          uv venv /app/venv",
            "        fi",
            "        uv pip install --python /app/venv/bin/python3 -r /home/hermeswebui/.hermes/plugins/hermes-wiki/requirements.txt",
